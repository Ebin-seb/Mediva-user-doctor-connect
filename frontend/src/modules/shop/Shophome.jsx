...
function Shophome() {
  ...
  const fetchshop = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/shop/shophome/${id}`);
      setdata(response.data.shop);
      localStorage.setItem('shopid', response.data.shop._id);
    } catch (e) {
      console.log(e);
    }
  };
  ...
  const isTabActive = (tabPath) => {
    if (tabPath === '') {
      return location.pathname === `/shopehome/${id}`;
    }
        ...
        <Container fluid>
          ...
          <div className="ms-auto d-flex align-items-center gap-3">
            <div className="text-end text-white">
              ...
              <small className="text-white-50">Shop Manager</small>
            </div>
