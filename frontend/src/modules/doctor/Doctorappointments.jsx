...
function Doctorappointments() {
    ...
    useEffect(() => {
        const fetchAppointments = async () => {
            try {
                const response = await axios.get(`http://localhost:8000/api/doctor/appointments/${doctorid}`)
    ...
    const updateAppointmentStatus = async (appointmentId, newStatus) => {
        try {
            await axios.patch(`http://localhost:8000/api/doctor/status/${appointmentId}`, {
                status: newStatus
            });
